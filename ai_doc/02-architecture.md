...
3. The gateway resolves the family, provider, model, and any optional inner provider.
   For open threads, if a request omits `provider` and `model`, the resolver now prefers the latest successful message route stored in message metadata, then the latest successful model gateway log for legacy conversations, before falling back to the thread-level route. This lets mid-conversation provider/model overrides continue naturally on later turns. `/dayend` remains a family-compaction operation and does not inherit the open thread's last chat route.
...
- Forced `/dayend` compaction always uses the family compaction settings; normal chat `provider` and `model` request overrides are ignored for this command. Automatic threshold compaction uses the same family compaction settings.
...
- Compacted memory messages record the resolved family compaction provider/model metadata for diagnostics, and forced `/dayend` logs reflect that actual compaction route when one is used.
...
- Assistant turns now persist the resolved `provider_id`, `provider`, `provider_model_id`, and `model` inside message metadata so later open-thread turns can resume the latest successful route even when the request omits overrides.
